...
import { RangeFilter, RangeValue } from "./types/filters-types";
...
const PriceSliderPip: React.FC<PriceSliderPipProps> = ({
  ...
  const pip = React.useMemo(() => {
    if (animation === "height") {
      return (
        <span
          className="relative h-12 w-1 rounded-full bg-default-200 after:absolute after:bottom-0 after:h-0 after:w-full after:rounded-full after:bg-red after:transition-all after:!duration-500 after:content-[''] data-[in-range=true]:after:h-full"
      ...
      <div className={cn("flex flex-col gap-3 ", className)}>
        ...
        <div className="flex items-center ">
          <Input
            ...
            startContent={<p className="text-white">&#8381;</p>}
            type="number"
            value={`${value[0]}`}
